...
describe('safetyStockMethods', () => {
  ...
  describe('agreement with the functions it describes', () => {
    ...
    it('marks serviceLevel required only on the statistical family', () => {
      const statHasServiceLevel = safetyStockMethods('statistical')
        .filter((m) => m.id !== 'max-minus-average')
        .every((m) => m.parameters.some((p) => p.name === 'serviceLevel' && p.required))
      expect(statHasServiceLevel).toBe(true)
